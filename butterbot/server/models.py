...
class MEvent(models.Model):
    ...
    champ = models.ForeignKey(MChamp , null=True , db_index=True,on_delete=None)
    res = models.CharField(max_length=50 , null=True)
    p1 = models.ForeignKey(MPlayer , related_name='mplayer1' , null=True , db_index=True,on_delete=None)
    p2 = models.ForeignKey(MPlayer , related_name='mplayer2' , null=True , db_index=True,on_delete=None)
...
class PlayerSetStats(models.Model):
    ...
    def __unicode__(self):
        return '{t:%s,sc:0,tp:%s,tw:%s,s1w:%s,s1ws2l:%s,s1wml:%s,s1ws2lmw:%s,s1l:%s,s1ls2w:%s,s1lmw:%s,s1ls2wml:%s}' % (
        self.sc , self.tp , self.tw , self.s1w , self.s1ws2l , self.s1wml , self.s1ws2lmw , self.s1l , self.s1ls2w ,
        self.s1lmw , self.s1ls2wml)
