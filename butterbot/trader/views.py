...
from trader.models import BFChamp , BFEvent , BFPlayer , BFOdds
    ...
    def get(self , request):
        ...
        if request.method == 'GET':
            ...
            if 'evid' in request.GET and 'isip' in request.GET:
                ...
                figure_or_data = [Scatter(x=xr , y=y1r , mode='lines' , name=event.pid1.name) ,
...
class BFApiIds(View):
    ...
    def clear_old(self , start):
        ...
        mcn = mchamps.count()
        mchamps.delete()
        ret = 'CLEARED: %s champs, %s events, %s odds, %s mchamps, %s mevents' % (bfcn , evn , oddn , mcn , mevn) if (
                                                                                                                                 bfcn + evn + oddn + mcn + mevn) > 0 else 'CLEARED: none'
        return ret